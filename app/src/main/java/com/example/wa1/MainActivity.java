...
public class MainActivity extends AppCompatActivity {
    ...
    private ArrayList<Object> afterSavedCityList, beforeSavedCityList;
    ...
    private final static String SAVED_LIST_KEY = "SAVED_LIST";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // init UI
        initUI();
        // retrieve the Searched List
        afterSavedCityList = new ArrayList<Object>();
        tinydb = new TinyDB(getApplicationContext());
        afterSavedCityList = tinydb.getListObject(SAVED_LIST_KEY, CityName.class);
        cityList = new ArrayList<CityName>(afterSavedCityList.size());
        if (afterSavedCityList.size() != 0) {
            for (int i = 1; i < afterSavedCityList.size(); i++) {
                cityList.add((CityName) afterSavedCityList.get(i));
            }
        }

        // test on Emulator
        cityName = getCityName(105.847130, 21.030653);
        cityList.add(0, new CityName(cityName));
        addBottomDots(0);
    ...
    // save the searched list
    @Override
    protected void onPause() {
        super.onPause();
        beforeSavedCityList = new ArrayList<Object>();
        for (CityName name : cityList) {
            beforeSavedCityList.add((Object) name);
        }
        tinydb.putListObject(SAVED_LIST_KEY, beforeSavedCityList);
    }
            ...
            public void onClick(View v) {
                ...
                mSearchDialog.setPositiveButton("Search", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (cityNameFromAlert.isEmpty()) {
                            displayMessage(ToastPerfect.WARNING, "Please enter an city name!");
                        } else {
